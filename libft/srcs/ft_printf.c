...
/*   Updated: 2022/07/29 17:17:24 by fmauguin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_printf(const char *line, ...)
{
	...
	while (line[i] && len >= 0)
	{
		if (line[i] != '%' || set_format(&format, &line[i], &i))
			__write(ft_printf_buffer(1, &line[i], 1), &len);
		else
			__write(do_convertion(1, &format, args), &len);
		i++;
	}
	va_end(args);
	__write(ft_printf_buffer(1, NULL, -1), &len);
	return (len);
}
