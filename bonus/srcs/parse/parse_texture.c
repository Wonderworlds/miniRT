...
/*   Updated: 2022/09/16 02:02:05 by fmauguin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	parse_vol_texture2(t_scene *scene, t_vol *vol,
					char *str, size_t *i)
{
...
static void	parse_pl_texture2(t_scene *scene, t_plane *pl,
					char *str, size_t *i)
{
	const char		*tbd[4] = {"texture:", "bumpmap:", "disruption:",
		"specular:"};
...
void	parse_vol_texture(t_scene *scene, t_vol *vol, char *str)
{
	...
	while (str[i] && str[i] != '\n')
	{
		...
		{
			i++;
			parse_vol_texture2(scene, vol, str, &i);
			str += i;
			i = 0;
		}
		else
...
void	parse_pl_texture(t_scene *scene, t_plane *pl, char *str)
{
	...
	while (str[i] && str[i] != '\n')
	{
		...
		{
			i++;
			parse_pl_texture2(scene, pl, str, &i);
			str += i;
			i = 0;
		}
		else
