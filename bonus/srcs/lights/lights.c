...
/*   Updated: 2022/09/12 16:33:35 by amahla           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		col_cpy(&lights->col, &light_specular);
		...
		{
			apply_light(&hit_from_camera, &dir_light, lights, &diffuse);
			add_coeficient(&specular,
				phong_reflection(hit_from_camera, dir_light, lights, scene->cam),
				&lights->col);
			c_mult(&specular, &lights->col, &light_specular[1]);
